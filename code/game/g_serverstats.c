...
void G_WriteXMLStats ( void ) {
	...
	writeToFile ( va ( "<match datetime=\"%i/%02i/%02i %02i:%02i:%02i\" duration=\"%i\" map=\"%s\" type=\"%s\" isTeamGame=\"%s\" instagib=\"%i\" rocketsOnly=\"%s\" reducedLightning=\"%s\" reducedRail=\"%s\" reducedMachinegun=\"%s\" aftershockRevision=\"%i\" g_aftershockPhysic=\"%i\" serverdemo=\"%s\">\n\n",
	                   ...
	                   g_instantgib.integer, boolToChar ( g_rockets.integer == 1 ), boolToChar( g_reduceLightningDamage.integer == 1 ), boolToChar( g_reduceRailDamage.integer == 1 ), boolToChar( g_reduceMachinegunDamage.integer == 1 ), REVISION, g_aftershockPhysic.integer, boolToChar( ( g_gametype.integer == GT_TOURNAMENT ) && g_autoServerDemos.integer ) ), &f );
