...
void AddScore( gentity_t *ent, vec3_t origin, int score ) {
	...
	if( ( g_gametype.integer == GT_ELIMINATION || g_gametype.integer == GT_CTF_ELIMINATION ) && !level.roundRespawned )
		return;
...
//unlagged - backward reconciliation #2
	...
	if (attacker && attacker->client) {
		...
		if ( attacker == self || OnSameTeam (self, attacker ) ) {
			...
			G_CheckDeniedReward( attacker, self );
			...
			if( ( self->client->spawnTime + 1000 > level.time ) && ( g_gametype.integer != GT_ELIMINATION && g_gametype.integer != GT_CTF_ELIMINATION )){
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	if ( attacker->client && client
		...
		if ( OnSameTeam( targ, attacker ) ) {
			...
			if( g_friendlyFire.value < 1.0 && g_friendlyFire.value >= 0.0)
