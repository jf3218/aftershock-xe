...
int Team_TouchOurFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	if( g_gametype.integer == GT_1FCTF ) {
	    ...
	    if( team == TEAM_RED ){
		    ...
		    trap_SendServerCommand( -1, va("screenPrint \"^3Red captured the flag in %i:%i:%03i\"", mins, secs, msecs) );
		    level.blueFlagTaken = -1;
	    }
	    else if( team == TEAM_BLUE ){
		    ...
		    PrintMsg( NULL, "Flag held for %i:%i:%i\n", mins, secs, msecs );
		    trap_SendServerCommand( -1, va("screenPrint \"^3Blue captured the flag in %i:%i:%03i\"", mins, secs, msecs) );
		    level.redFlagTaken = -1;
	    }
	...
	// add the sprite over the player's head
	other->client->ps.eFlags &= ~(EF_AWARD_IMPRESSIVE | EF_AWARD_EXCELLENT | EF_AWARD_GAUNTLET | EF_AWARD_ASSIST | EF_AWARD_DEFEND | EF_AWARD_CAP | EF_AWARD_AIRROCKET | EF_AWARD_AIRGRENADE );
	...
	for (i = 0; i < g_maxclients.integer; i++) {
		...
		if (player->client->sess.sessionTeam !=
			...
			if (player->client->pers.teamState.lastreturnedflag + 
				...
				// add the sprite over the player's head
				player->client->ps.eFlags &= ~(EF_AWARD_IMPRESSIVE | EF_AWARD_EXCELLENT | EF_AWARD_GAUNTLET | EF_AWARD_ASSIST | EF_AWARD_DEFEND | EF_AWARD_CAP | EF_AWARD_AIRROCKET | EF_AWARD_AIRGRENADE );
