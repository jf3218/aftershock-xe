...
//		trap_S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.noAmmoSound );
    ...
    case EV_USE_ITEM14:
        ...
        // other events
        //
    case EV_PROJECTILE_TELEPORT_IN:
        DEBUGNAME("EV_PROJECTILE_TELEPORT_IN");
        trap_S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.teleInProjectileSound);
        CG_SpawnEffect(position);
        break;

    case EV_PROJECTILE_TELEPORT_OUT:
        DEBUGNAME("EV_PROJECTILE_TELEPORT_OUT");
        trap_S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.teleOutProjectileSound);
        CG_SpawnEffect(position);
        break;
    ...
    case EV_GLOBAL_TEAM_SOUND:	// play from the player's head so it never diminishes
    {
        ...
        case GTS_RED_TAKEN: // CTF: red team took blue flag, 1FCTF: blue team took the neutral flag
            ...
            if (cg.snap->ps.powerups[PW_BLUEFLAG] || cg.snap->ps.powerups[PW_NEUTRALFLAG]) {
                ...
                if (cgs.clientinfo[cg.clientNum].team == TEAM_BLUE || ( cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR && cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE ) ) {
#ifdef MISSIONPACK
                ...
                else if (cgs.clientinfo[cg.clientNum].team == TEAM_RED || ( cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR && cg.snap->ps.persistant[PERS_TEAM] == TEAM_RED ) ) {
#ifdef MISSIONPACK
                    if (cgs.gametype == GT_1FCTF)
